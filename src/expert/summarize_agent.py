...
def _format_challenge_evaluation(challenge_eval: Dict[str, Any], challenge_spec: Dict[str, Any], 
                                  utterances_labeled: list, key_moments: Dict[str, Any]) -> Dict[str, Any]:
    """challenge_eval을 요청된 형식으로 변환"""
    if not challenge_eval:
        return {}
    
    # challenge_spec에서 정보 가져오기
    challenge_id = challenge_spec.get("challenge_id", "chg-unknown")
    title = challenge_spec.get("title", "")
    goal_description = challenge_spec.get("goal", "")
    required_count = challenge_spec.get("required_count", 0)
    
    # challenge_eval에서 정보 가져오기
    is_success = challenge_eval.get("challenge_met", False)
    notes = challenge_eval.get("feedback", "")
    
    # evidences 생성 (key_moments의 pattern_examples 활용)
    evidences = []
    if isinstance(key_moments, dict):
        pattern_examples = key_moments.get("pattern_examples", [])
        for idx, pattern in enumerate(pattern_examples[:3], 1):
            dialogue_list = pattern.get("dialogue", [])
            if dialogue_list:
                # dialogue를 요청된 형식으로 변환
                formatted_dialogue = []
                for d in dialogue_list:
                    speaker = d.get("speaker", "").lower()
                    if speaker in ["mom", "parent", "부모"]:
                        speaker = "parent"
                    elif speaker in ["chi", "child", "아이"]:
                        speaker = "child"
                ...
                # matched 여부는 패턴의 문제 설명 기반으로 추정
                matched = "긍정" in pattern.get("suggested_response", "") or "칭찬" in pattern.get("suggested_response", "")
                
                evidences.append({
                    "index": idx,
                    "dialogue": formatted_dialogue,
                    "matched": matched,
                    "reason": pattern.get("problem_explanation", "") or pattern.get("suggested_response", "")
                })
    
    # matched_count 계산
    matched_count = sum(1 for e in evidences if e.get("matched", False))
