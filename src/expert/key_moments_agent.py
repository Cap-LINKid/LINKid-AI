...
_KEY_MOMENTS_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        (
            ...
            "출력 형식 규칙:\n"
            "- 중요한 단어나 구절은 **강조할 텍스트**와 같이 마크다운 볼드체(**텍스트**)로 표시하세요.\n"
            "- reason, better_response, problem_explanation, suggested_response 필드에서 핵심 개념, 핵심 원인, 핵심 조언을 볼드 처리하세요.\n"
            "- dialogue[].text(대화 원문)는 절대 볼드 처리하지 말고 원문 그대로 출력하세요.\n"
            "- 볼드체는 과도하게 사용하지 말고, 각 필드당 1~2개 핵심 구절만 강조하세요.\n"
            "- 서술을 읽기 쉽게 구조화하세요. reason은 짧게 끊어 읽기 쉽게 작성하세요.\n"
            "- 한 문단에 모든 내용을 몰아넣지 말고, 문장을 짧게 끊어 읽기 쉽게 작성하세요.\n"
            "- 불필요한 반복(같은 의미의 재진술)을 피하고 핵심만 남기세요.\n\n"
...
_IMPROVE_NEEDS_IMPROVEMENT_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        (
            ...
            "요구사항:\n"
            "1. 'reason'에는 전문가 조언의 핵심 내용을 직접 인용하거나 요약하여 포함하되, 핵심을 잘 드러내도록 구성하세요.\n"
            "2. 전문가 조언의 구체적인 문장, 원칙, 설명 중 필요한 핵심만 간결하게 요약하여 'reason'에 반영하세요.\n"
...
_IMPROVE_POSITIVE_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        (
            ...
            "'reason'(왜 이 순간이 좋은지)을 구조화하여 가독성 좋게 개선해서 작성하세요.\n\n"
            "요구사항:\n"
            "1. 'reason'에는 반드시 전문가 조언의 핵심 내용을 직접 인용하거나 요약하여 포함해야 합니다.\n"
            "2. 전문가 조언의 구체적인 문장, 원칙, 설명을 그대로 인용하거나 요약하여 'reason'에 반영하세요.\n"
            "3. 이 순간이 아이의 정서, 자존감, 부모-자녀 관계에 어떤 긍정적인 영향을 주는지 분명하게 설명하세요. 단, 문장을 짧게 끊어 읽기 쉽게 작성하세요.\n"
            "4. 모든 서술은 반말이 아닌 **존댓말(예: \"~합니다\", \"~합니다.\")** 체로 공손하게 작성하세요.\n"
            "5. 한국어로 작성하세요.\n\n"
            "출력 형식 규칙:\n"
            "- 중요한 단어나 구절은 **강조할 텍스트**와 같이 마크다운 볼드체(**텍스트**)로 표시하세요.\n"
    ...
    (
        ...
        (
            ...
            "전문가 조언의 내용을 반영하여 구조화된 'reason'을 작성해주세요."
        ),
    ),
])
