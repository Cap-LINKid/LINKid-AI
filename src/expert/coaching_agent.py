...
_COACHING_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        (
            ...
            '  \"challenge\": {{\n'
            '    \"title\": \"챌린지 제목 (가장 중요한 부정적 패턴을 극복하기 위한 챌린지 명칭)\",\n'
            ...
            "2) 각 action은 반드시 다음 형식으로 작성해야 함:\n"
            "   - **상황 설명**: 어떤 구체적인 상황에서 적용할지 명시 (예: '아이가 고집을 부릴 때', '아이가 화가 나서 소리를 지를 때', '아이가 부모의 지시를 거부할 때')\n"
            "   - **원칙 설명**: 그 상황에서 어떤 방식으로 말하고 행동할지 원칙을 제시 (예: '비판하기보다는 아이의 감정을 먼저 수용하기', '지시하기보다는 공감과 이해를 표현하기')\n"
            "   - **예시 문장 제공**: 해당 원칙을 보여주는 구체적인 말문장 예시 1-2개 (예: \"예: '그런 마음이었구나, 궁금한 건 이해해' 또는 '지금 화가 많이 났구나'\")\n"
            "   - 각 action은 다음 세 가지를 동시에 반영해야 함:\n"
            "     * 패턴 정보 (예: '비판적 반응', '과도한 지시' 등)\n"
            "     * 실제 대화에서 반복된 구체 장면 (예: 용돈, 물건 뺏기, 고집 부림, 협박 발화 등)\n"
            "     * 전문가 레퍼런스에서 제시한 구체적 방법 (말문장 예시, 태도 변화, 훈육 원칙 등)\n"
            "   - **중요**: 특정 발화를 직접적으로 '이렇게 말하세요'라고 지시하지 말고, 상황과 원칙을 설명한 후 예시를 제공하는 형태로 작성할 것.\n"
            "   - 예시 형식: \"[상황 설명]에는 [원칙 설명하기]. 예: '[예시 문장1]' 또는 '[예시 문장2]'\"\n"
            "   - **문체**: action 문장은 '~하기'와 같은 지시형으로 작성할 것. '~합니다', '~대화합니다'와 같은 서술형을 사용하지 말 것. '경향으로 대화하기'와 같은 고정된 문구를 사용하지 말고, 각 상황에 맞는 자연스러운 지시형 문장으로 작성할 것.\n"
    ...
    (
        ...
        (
            ...
            "- actions는 다음 형식으로 작성해주세요:\n"
            "  * 각 action은 '상황 설명 + 원칙 설명하기 + 예시 문장' 형태로 구성\n"
            "  * 특정 발화를 직접적으로 '이렇게 말하세요'라고 지시하지 말고, 어떤 상황에서 어떤 방식으로 말할지 원칙을 제시한 후 예시를 제공\n"
            "  * 문체: '~하기'와 같은 지시형으로 작성. '~합니다', '~대화합니다'와 같은 서술형을 사용하지 말 것. '경향으로 대화하기'와 같은 고정된 문구를 사용하지 말고, 각 상황에 맞는 자연스러운 지시형 문장으로 작성할 것.\n"
            "  * 예시: \"아이가 감정을 표현할 때에는 비판하기보다는 그 감정을 먼저 수용하기. 예: '그런 마음이었구나, 힘들었겠네' 또는 '지금 화가 많이 났구나'\"\n"
            "- goal은 전문가 조언을 기존 대화의 맥락을 고려해서 실제 부모-자녀 대화에 적용하는 형태로, "
            "상황과 경향을 중심으로 작성해주세요.\n"
