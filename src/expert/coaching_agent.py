...
_COACHING_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        (
            ...
            "   - 예시 형식: \"[상황 설명]에는 [경향/원칙 설명]. 예: '[예시 문장1]' 또는 '[예시 문장2]'\"\n"
    ...
    (
        ...
        (
            ...
            "  * 각 action은 '상황 설명 + 경향/원칙 설명 + 예시 문장' 형태로 구성\n"
            "  * 특정 발화를 직접적으로 '이렇게 말하세요'라고 지시하지 말고, 어떤 상황에서 어떤 경향으로 말할지 원칙을 제시한 후 예시를 제공\n"
            "  * 예시: \"아이가 고집을 부릴 때에는 비판하기보다는 아이의 감정을 먼저 수용하는 경향으로 대화합니다. 예: '그런 마음이었구나, 궁금한 건 이해해' 또는 '지금 화가 많이 났구나'\"\n"
