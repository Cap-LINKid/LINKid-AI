...
_COACHING_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        (
            ...
            "   - 왜 이 챌린지를 선택했는지 (상황에 대한 부드러운 묘사, 감지된 패턴에 대한 간단한 언급)\n"
            "   - **톤 지침**: summary는 부드럽고 건설적인 톤으로 작성하세요.\n"
            "     - 부정적 표현을 완화: '문제가 있습니다' → '개선의 여지가 있습니다', '반복되고 있습니다' → '일부 관찰되었습니다'\n"
            "     - 비난하거나 지적하는 톤보다는, 함께 개선해 나가는 지원적 톤을 유지하세요.\n"
            "   - **빈도 언급 주의**: 패턴 빈도(occurrences)나 '반복', '많이' 같은 표현은 다음 조건에서만 사용하세요:\n"
            "     - patterns 정보에 명시적으로 'occurrences' 또는 빈도 수치가 제공된 경우\n"
            "     - key_moments의 pattern_examples에 'occurrences' 값이 3회 이상인 경우\n"
            "     - 위 조건이 충족되지 않으면 빈도 언급 없이 패턴 자체만 간단히 언급하세요.\n"
            "   - **반드시 전문가 레퍼런스 내용을 인용**:\n"
    ...
    (
        ...
        (
            ...
            "- **summary 작성 시 주의사항**:\n"
            "  - 부드럽고 건설적인 톤을 유지하세요. 비난하거나 지적하는 표현보다는 함께 개선해 나가는 지원적 톤을 사용하세요.\n"
            "  - '반복되고 있습니다', '많이', '과도하게' 같은 빈도 표현은 patterns에 occurrences 값이 명시적으로 3회 이상인 경우에만 사용하세요.\n"
            "  - 그 외의 경우에는 '일부 관찰되었습니다', '개선의 여지가 있습니다' 같은 부드러운 표현을 사용하세요.\n"
